...
foreach ($eachDS in ($Datastores | Where-Object {$_.State -eq "Available"})) {
   ...
   $fileList = @(Get-ChildItem -Path "$FilePath" -ErrorAction SilentlyContinue | Select-Object Name, FolderPath, FullName)
...
# Changelog
## 1.0 : Initial Version
## 1.6 : Added the -ErrorAction SilentlyContinue parameter to the Get-ChildItem calls to prevent errors for files that no longer exist.
